#include "AbilitySystem/Enemy/GAS/GA_Enemy_SpawnTornadoHazard.h"
...
UGA_Enemy_SpawnTornadoHazard::UGA_Enemy_SpawnTornadoHazard(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	TriggerData.TriggerTag = RetrieveGameplayTags::GameplayEvent_Enemy_SpecialAttack_Tornado;
	TriggerData.TriggerSource = EGameplayAbilityTriggerSource::GameplayEvent;
	AbilityTriggers.Add(TriggerData);
}

void UGA_Enemy_SpawnTornadoHazard::ActivateAbility(
	...
	const FGameplayEventData* TriggerEventData)
{
	...
	ScheduleTornadoSpawns();
}

void UGA_Enemy_SpawnTornadoHazard::EndAbility(
	const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo,
	const FGameplayAbilityActivationInfo ActivationInfo,
...
UAnimMontage* UGA_Enemy_SpawnTornadoHazard::ResolveAttackMontage(
...
void UGA_Enemy_SpawnTornadoHazard::ScheduleTornadoSpawns()
{
	...
	SpawnTimerHandles.SetNum(PendingSpawnCount);
	...
	for (int32 EntryIndex = 0; EntryIndex < SpawnEntries.Num(); ++EntryIndex)
	{
		const float AdjustedDelay = Combat
			? Combat->GetAttackDelay(SpawnEntries[EntryIndex].SpawnDelay)
			: FMath::Max(0.f, SpawnEntries[EntryIndex].SpawnDelay);

		if (AdjustedDelay <= 0.f)
		{
			SpawnTornado(EntryIndex);
			continue;
		}

		World->GetTimerManager().SetTimer(
			SpawnTimerHandles[EntryIndex],
			FTimerDelegate::CreateUObject(this, &ThisClass::SpawnTornado, EntryIndex),
			AdjustedDelay,
			false);
	}
}

void UGA_Enemy_SpawnTornadoHazard::SpawnTornado(int32 SpawnEntryIndex)
{
	...
	if (Avatar && World && HasAuthority(&GetCurrentActivationInfoRef()))
	{
		const FVector RequestedLocation = Avatar->GetActorLocation()
			+ Avatar->GetActorRotation().RotateVector(SpawnEntries[SpawnEntryIndex].SpawnOffset);
		...
		if (ResolveGroundLocation(RequestedLocation, GroundLocation))
		{
			...
			World->SpawnActor<AEnemyTornadoHazard>(
				TornadoHazardClass,
				GroundLocation,
				SpawnRotation,
				SpawnParams);
...
bool UGA_Enemy_SpawnTornadoHazard::ResolveGroundLocation(
	...
	FVector& OutGroundLocation) const
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(SpawnTornadoHazard), false, Avatar);
...
void UGA_Enemy_SpawnTornadoHazard::TryFinishAbility()
{
...
void UGA_Enemy_SpawnTornadoHazard::FinishAbility(bool bWasCancelled)
{
	if (IsActive())
	{
...
void UGA_Enemy_SpawnTornadoHazard::OnMontageCompleted()
{
	bMontageFinished = true;
	TryFinishAbility();
}

void UGA_Enemy_SpawnTornadoHazard::OnMontageInterrupted()
{
	FinishAbility(true);
}
